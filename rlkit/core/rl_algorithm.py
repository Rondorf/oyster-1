...
class MetaRLAlgorithmImage(metaclass=abc.ABCMeta):
    def __init__(
            self,
            env,
            agent,
    ...
    def collect_data(self, num_samples, resample_z_rate, update_posterior_rate, add_to_enc_buffer=True):
        ...
        while num_transitions < num_samples:
            ...
            if add_to_enc_buffer:
                self.enc_replay_buffer.add_paths(self.task_idx, paths)
            if update_posterior_rate != np.inf:
                image_context, context = self.sample_context(self.task_idx)
                self.agent.infer_posterior(image_context, context)
        self._n_env_steps_total += num_transitions
        gt.stamp('sample')
    ...
    def collect_paths(self, idx, epoch, run):
        ...
        while num_transitions < self.num_steps_per_eval:
            ...
            num_trajs += 1
            if num_trajs >= self.num_exp_traj_eval:
                self.agent.infer_posterior(self.agent.image_context, self.agent.context)
    ...
    def evaluate(self, epoch):
        ...
        for idx in indices:
            ...
            paths = []
            for _ in range(self.num_steps_per_eval // self.max_path_length):
                image_context, context = self.sample_context(idx)
                self.agent.infer_posterior(image_context, context)
                p, _ = self.sampler.obtain_samples(deterministic=self.eval_deterministic, max_samples=self.max_path_length,
                                                        accum_context=False,
                                                        max_trajs=1,
